    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 pt-24 pb-8 text-center">
          <p className="text-foreground-muted text-lg">Recipe not found.</p>
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 pt-24 pb-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            <div>
              <button onClick={() => router.push('/recipes')} className="text-foreground-muted hover:text-foreground text-sm mb-2 inline-block">
            ...
            <div className="flex gap-2">
              ...
              {isLastStep && (
                <button
                  ...
                  className="px-4 py-2 rounded-lg text-sm font-semibold bg-gradient-to-r from-green-500 to-emerald-600 text-foreground hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] transition"
                >
                  I Made This!
                </button>
              )}
            ...
            <div className={showChat ? 'lg:col-span-3' : ''}>
                ...
                {stepTimers.length > 0 && (
                  <div className="mt-6 space-y-3">
                    {stepTimers.map(timer => (
                      <div key={timer.id} className="flex items-center gap-3 bg-surface-raised/50 rounded-lg p-3">
                        ...
                        <div className="flex-1">
                          <p className="text-foreground-muted text-sm">{timer.label}</p>
                        </div>
                        <button
                        ...
                        <button
                          ...
                          className="px-2 py-1.5 rounded-md text-sm text-foreground-muted hover:text-foreground hover:bg-white/10 transition"
                        >
                          Reset
                        </button>
                      </div>
              ...
              <div className="flex justify-between items-center mb-6">
                <button
                  ...
                  className="px-6 py-3 rounded-lg text-sm font-semibold border border-white/20 text-foreground hover:bg-white/10 disabled:opacity-30 transition"
                >
                  Previous
                </button>
                ...
                {isLastStep ? (
                  <button
                    onClick={handleIMadeThis}
                    className="px-6 py-3 rounded-lg text-sm font-semibold bg-gradient-to-r from-green-500 to-emerald-600 text-foreground hover:shadow-[0_0_20px_rgba(16,185,129,0.4)] transition"
                  >
                    I Made This!
                  </button>
                ) : (
                  <button
                    ...
                    className="px-6 py-3 rounded-lg text-sm font-semibold bg-primary text-primary-foreground hover:bg-primary-hover transition"
                  >
                    Next
                  </button>
                )}
              ...
              {timers.length > 0 && (
                <div className="glass-card rounded-xl p-4 mb-6">
                  <h3 className="text-sm font-semibold text-foreground-muted uppercase tracking-wider mb-3">All Timers</h3>
                        ...
                        className={`text-left p-2 rounded-lg text-sm transition ${
                          timer.isRunning
                            ...
                            : timer.remainingSeconds === 0
                              ...
                              : 'bg-surface-raised/50 border border-border hover:border-primary/30'
                        }`}
                      >
                        <span className="text-foreground-muted text-xs">{timer.label}</span>
                        <span className={`block font-mono font-bold ${
                          timer.isRunning ? 'text-[#f59e0b]' : timer.remainingSeconds === 0 ? 'text-green-400' : 'text-foreground'
                        }`}>
                          {formatTime(timer.remainingSeconds)}
                        </span>
                      </button>
              ...
              <div className="glass-card rounded-xl p-4">
                <h3 className="text-sm font-semibold text-foreground-muted uppercase tracking-wider mb-3">Ingredients</h3>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                  {recipe.ingredients.map((ing, idx) => (
                    <p key={idx} className="text-sm text-foreground">
                      <span className="text-primary">{ing.quantity} {ing.unit}</span> {ing.name}
                    </p>
                  ))}
                </div>
              </div>
            ...
            {showChat && (
              <div className="lg:col-span-2 glass-card rounded-xl flex flex-col" style={{ maxHeight: 'calc(100vh - 120px)' }}>
                <div className="p-4 border-b border-border">
                  <h3 className="text-foreground font-semibold">Cooking Assistant</h3>
                    ...
                    <div className="flex justify-start">
                      <div className="bg-surface-raised/50 rounded-lg p-3">
                        <div className="flex space-x-1.5">
                          ...
                          <div className="w-1.5 h-1.5 bg-foreground-muted rounded-full animate-bounce" style={{ animationDelay: '0.2s' }} />
                          <div className="w-1.5 h-1.5 bg-foreground-muted rounded-full animate-bounce" style={{ animationDelay: '0.4s' }} />
                        </div>
                      </div>
                    </div>
                  )}
                ...
                <form onSubmit={handleSendChat} className="p-3 border-t border-border">
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={chatInput}
                      ...
                      className="flex-1 px-3 py-2 text-sm border border-border rounded-lg bg-surface-raised/50 text-foreground focus:outline-none focus:ring-1 focus:ring-primary disabled:opacity-50"
                    />
                    <button
                      type="submit"
                      disabled={chatLoading || !chatInput.trim()}
                      className="px-4 py-2 text-sm font-semibold bg-primary text-primary-foreground rounded-lg disabled:opacity-50 transition"
                    >
                      Send
                    </button>
                  </div>
        ...
        {showDeduction && (
          <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
            <div className="glass-card rounded-xl p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto">
              <h3 className="text-xl font-bold text-foreground mb-2">Great job cooking!</h3>
              ...
              <div className="space-y-2 mb-6">
                {recipe.ingredients.map((ing, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center justify-between bg-surface-raised/50 rounded-lg p-3">
                      <div>
                        <p className="text-foreground text-sm font-medium">{ing.name}</p>
                        <p className="text-foreground-muted text-xs">{ing.quantity} {ing.unit}</p>
                      </div>
                      {match ? (
                        ...
                        <span className="text-xs px-2 py-1 bg-white/10 text-foreground-muted rounded">
                          Not in inventory
                        </span>
                      )}
                    </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={deducting}
                  className="flex-1 px-4 py-2 rounded-lg text-sm font-semibold bg-gradient-to-r from-green-500 to-emerald-600 text-foreground disabled:opacity-50 transition"
                >
                  {deducting ? 'Deducting...' : 'Deduct & Finish'}
                </button>
                <button
                  onClick={() => { setShowDeduction(false); router.push('/recipes'); }}
                  className="flex-1 px-4 py-2 rounded-lg text-sm font-semibold bg-white/10 text-foreground hover:bg-white/20 transition"
                >
                  Skip
                </button>
              </div>
